                ...
                for reservations in ec2details['Reservations']:
                    for insttype in reservations['Instances']:
                        ...
                        if instancetype[0] in freezedInstanceType:
                            ...
                            #response = client.reboot_instances(InstanceIds=['string',],DryRun=True)
                            #response = cw.put_metric_alarm(AlarmName="%s %s System Check Failed" % (name_tag, matchedInstance), AlarmDescription='Status Check Failed (System) for 5 Minutes', ActionsEnabled=True, AlarmActions=[ec2_sns, "arn:aws:automate:%s:ec2:recover" % currentregion], MetricName='StatusCheckFailed_System', Namespace='AWS/EC2', Statistic='Average', Dimensions=[{ 'Name': 'InstanceId', 'Value': matchedInstance }, ], Period=60, EvaluationPeriods=5, Threshold=1.0, ComparisonOperator='GreaterThanOrEqualToThreshold')
